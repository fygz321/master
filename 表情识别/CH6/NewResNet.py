...
class Bottleneck(nn.Module): # 卷积3层，F(X)和X的维度不等
    ...
    # groups:分组卷积，可减少参数量，且不容易过拟合(类似正则化)
    # 卷积参数量计算公式:(in_channels * out_channels * kernel_size^2) / groups
...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.in_channel = 64 # 第一层卷积输出特征矩阵的深度,也是后面输入特征的深度
        ...
        if self.include_top:
            self.avgpool = nn.AdaptiveAvgPool2d((1,1)) # 自适应平局池化下采样，无论输入矩阵的shape为多少，output size 的高宽均为1*1
            # 使矩阵展平为向量,如(W,H,C)->(1,1,W*H*C),深度为W*H*C
            # 思考为什么*block.expansion
            self.fc = nn.Linear(512*block.expansion, num_classes) # 全连接层，512*block.expansion为输入深度
    ...
    def _make_layer(self, block, channel, block_num, stride=1):
        ...
        # 输入通道每经过一组残差块,就在原来的基础上*block.expansion,从而满足残差块之间通道数量自洽
        self.in_channel = channel * block.expansion

        #后面的残差块不需要对X下采样
        #输入通道:self.in_channel = channel * block.expansion;输出:channel * block.expansion,下一层再传入给in_channels,产生闭合
        ...
        # 最后的输出通道数量:channel * block.expansion
